...
export async function deleteHeadline(id: string): Promise<void> {
    const res = await fetch(`/api/news-headlines/${encodeURIComponent(id)}`, {
        method: 'DELETE',
    });
    if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(errorData.error || `Delete headline failed: ${res.status}`);
    }
}

export async function updateHeadline(id: string, data: { title: string; summary: string; source: string }): Promise<HeadlineItem> {
    const res = await fetch(`/api/news-headlines/${encodeURIComponent(id)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(errorData.error || `Update headline failed: ${res.status}`);
    }
    return res.json();
}
