                ...
                configureServer(server: any) {
                    server.middlewares.use(async (req: any, res: any, next: any) => {
                        if (req.url?.startsWith('/api/scroll') || req.url?.startsWith('/api/prayer-request') || req.url?.startsWith('/api/testimonies') || req.url?.startsWith('/api/donations') || req.url?.startsWith('/api/news')) {
                            ...
                            const fetchDailyNews = async () => {
                                try {
                                    ...
                                    try {
                                        ...
                                        existing = JSON.parse(dataRaw);
                                    } catch (e) { }

                                    const fetchedArticles: any[] = [];

                                    // 1. NewsAPI.org
                                    const newsApiKey = env.NEWSAPI_KEY || env.NEWS_API_KEY;
                                    if (newsApiKey) {
                                        try {
                                            console.log(`[VITE-NEWS] Fetching from NewsAPI.org...`);
                                            ...
                                            if (json.status === 'ok' && json.articles) {
                                                json.articles.forEach((a: any) => fetchedArticles.push({
                                                    title: `${a.title} - ${a.source?.name || 'NewsAPI'}`,
                                                    content: a.url || a.description || 'Read more'
                                                }));
                                            }
                                        } catch (e) { console.error('[VITE-NEWS] NewsAPI error:', e); }
                                    }
                                    ...
                                    if (newsDataKey) {
                                        try {
                                            console.log(`[VITE-NEWS] Fetching from NewsData.io...`);
                                            ...
                                            if (json.status === 'success' && json.results) {
                                                json.results.forEach((a: any) => fetchedArticles.push({
                                                    title: a.title,
                                                    content: a.link || a.description || 'Read more'
                                                }));
                                            }
                                        } catch (e) { console.error('[VITE-NEWS] NewsData error:', e); }
                                    }
                                    ...
                                    if (gnewsKey) {
                                        try {
                                            console.log(`[VITE-NEWS] Fetching from GNews.io...`);
                                        ...
                                        } catch (e) { console.error('[VITE-NEWS] GNews error:', e); }
                                    }
                                    ...
                                    if (newItems.length > 0) {
                                        ...
                                        await fs.writeFile(newsPath, JSON.stringify(existing, null, 2), 'utf-8');
                                    }
                                    console.log(`[VITE-NEWS] Added ${addedCount} items.`);
                                } catch (error) {
                                    console.error('[VITE-NEWS] Error:', error);
                                }
                            };
                            ...
                            // Handler for /api/news/fetch
                            if (url === '/api/news/fetch') {
                                if (req.method === 'POST') {
                                    if (!verifyAuthHeader(req)) {
                                        res.statusCode = 401;
                                        res.end(JSON.stringify({ error: 'Unauthorized' }));
